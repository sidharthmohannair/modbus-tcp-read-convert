...
# Modbus Read and Convert using Subprocess and mbpoll

This script uses the `mbpoll` command-line utility to read Modbus registers and `subprocess` to capture the output.
...
def run_mb_poll():
    try:
        ...
        try:
            ...
            print(f"Raw output after kill: {output}")
            print(f"Raw error after kill: {error}")
...
```sh
python modbus_read_convert_subprocess.py
```

This script reads Modbus registers using the `mbpoll` utility and subprocess, parses the register values, and converts them to a floating-point number.
